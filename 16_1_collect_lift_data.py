...
for episode in range(num_episode):
    ...
    for step in range(300):
        ...
        if phase == "above":
            ...
            if obs["cube_pos"][2] > initial_cube_z + 0.1:
                phase = "hold"
    ...
    if cube_lifted:
        ...
        for t in range(len(episode_observations)):
            obs_sequence = episode_observations[t]

            action_chunk = episode_actions[t : t + 16]

            # episode action이 16개가 안되면 마지막 action padding
            if len(action_chunk) < 16:
                # [None, :] == reshape(1, n)
                padding = np.repeat(
                    action_chunk[-1][None, :], 16 - len(action_chunk), axis=0
                )

                action_chunk = np.concatenate([action_chunk, padding], axis=0)

            all_observations.append(obs_sequence)
...
np.savez(
    "lift_dataset_2obs_16actions.npz",
